...
class JSExtension(_PyV8.JSExtension):
    def __init__(self, name, source, callback=None, dependencies=[], register=True):
        _PyV8.JSExtension.__init__(self, js_escape_unicode(name), js_escape_unicode(source), callback, dependencies,
                                   register)
...
def convert(obj):
    ...
    if type(obj) == _PyV8.JSObject:
        return dict(
            [[str(k), convert(obj.__getattr__(str(k)))] for k in (obj.__dir__() if is_py3k else obj.__members__)])
...
class TestWrapper(unittest.TestCase):
    ...
    def testJSFunction(self):
        with JSContext() as ctxt:
            ...
            obj = ctxt.eval(
                "({ 'name': 'flier', 'hello': function (name) { return 'hello ' + name + ' from ' + this.name; }})")
            hello = obj.hello
    ...
    def testExceptionMapping(self):
        ...
        with JSContext(Global()) as ctxt:
            ...
            self.assertEqual("TypeError: int() argument must be a string or a number, not 'module'",
                             str(ctxt.locals.msg))
...
class TestEngine(unittest.TestCase):
    ...
    def testNativeExtension(self):
        ...
        extPy = JSExtension("hello/python", extSrc, lambda func: lambda name: "hello " + name + " from python",
                            register=False)
    ...
    def testBlock(self):
        class BlockChecker(TestAST.Checker):
            def onBlock(self, stmt):
                ...
                self.assertEqual(['%InitializeVarGlobal("i", 0);', '%InitializeVarGlobal("j", 0);'],
